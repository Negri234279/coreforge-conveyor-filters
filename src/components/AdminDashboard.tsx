...
function InfoDot({ tip }: { tip: string }) {
    return (
        ...
        >
            ?
            <span class="pointer-events-none absolute top-full left-1/2 z-20 mt-2 hidden w-64 -translate-x-1/2 rounded border border-slate-800 bg-slate-950/95 p-2 text-[11px] leading-snug whitespace-normal text-slate-200 shadow-lg group-hover:block peer-focus:block">
                {tip}
            </span>
        </span>
    )
...
function Stat({
    ...
    return (
        <div class="rounded border border-slate-800 bg-slate-900/60 px-3 py-2" title={tip}>
            ...
            {hint ? <div class="mt-0.5 font-mono text-[11px] text-slate-600">{hint}</div> : null}
        </div>
    )
}
                                ...
                                <tbody>
                                    {s.events.countsWeek.map((c) => (
                                        ...
                                        >
                                            ...
                                            <td class="py-1 text-right font-mono text-xs text-slate-400">
                                                {fmt(c.n)}
                                            </td>
                                        </tr>
                                    ))}
                            ...
                            <tbody>
                                {s.events.recent.map((e) => (
                                    ...
                                    >
                                        ...
                                        <td class="py-1 font-mono text-[11px] text-slate-600">
                                            {e.metadata ?? ''}
                                        </td>
                                    </tr>
                                ))}
